...
class CoSimulation:
    ep_path = Path(r'C:\Users\spili\Desktop\EnergyPlusV22-2-0')
    ...
    def callback_function(self, state):
        ...
        if season is not None and 8 <= self.api.exchange.hour(state) <= 19:
            ...
            self.evaluation_results.save_result(self.sim_time.strftime("2020-%m-%d %H:%M"), self.sim_time.strftime("2020-%m-%d"), self.total_people_count)
